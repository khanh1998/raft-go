...
func parseDeleteRequest(c *gin.Context) (EtcdCommandRequest, error) {
	key := c.Param("key")
	if key == "" {
		return EtcdCommandRequest{}, CreateError("key", errors.New("`key` can't be empty"))
	}
	...
	if err != nil {
		return EtcdCommandRequest{}, CreateError("prevExist", err)
	}
	...
	if err != nil {
		return EtcdCommandRequest{}, CreateError("prevIndex", err)
	}

	cmd.Prefix, err = getBool(c, "prefix")
	if err != nil {
		return EtcdCommandRequest{}, CreateError("prefix", err)
	}
...
func parseGetRequest(c *gin.Context) (EtcdCommandRequest, error) {
	...
	if err != nil {
		return EtcdCommandRequest{}, CreateError("wait", err)
	}

	cmd.WaitIndex, err = getInt(c, "waitIndex")
	if err != nil {
		return EtcdCommandRequest{}, CreateError("waitIndex", err)
	}
...
func parsePutRequest(c *gin.Context) (EtcdCommandRequest, error) {
	key := c.Param("key")
	if key == "" {
		return EtcdCommandRequest{}, CreateError("key", errors.New("can't be empty"))
	}
	...
	if err != nil {
		return EtcdCommandRequest{}, CreateError("ttl", err)
	}

	cmd.Refresh, err = getBool(c, "refresh")
	if err != nil {
		return EtcdCommandRequest{}, CreateError("refresh", err)
	}
