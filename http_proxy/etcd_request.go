...
func (e EtcdCommandRequest) ValidateGet() error {
	if e.Action != strings.ToLower(http.MethodGet) {
		return etcd.CreateError("action", errors.New("`action` must be `get`"))
	}

	if e.Key == "" {
		return etcd.CreateError("key", errors.New("`key` can't be empty string"))
	}

	if e.WaitIndex != nil {
		// index 0 is meaningless in Raft
		if *e.WaitIndex <= 0 {
			return etcd.CreateError("waitIndex", errors.New("`waitIndex` must be larger than 0"))
		}

		if e.Wait == nil || !(*e.Wait) {
			return etcd.CreateError("wait", errors.New("`waitIndex` must go together with `wait=true`"))
		}
	}
...
func (e EtcdCommandRequest) ValidatePut() error {
	if e.Action != strings.ToLower(http.MethodPut) {
		return etcd.CreateError("action", errors.New("`action` must be `put`"))
	}
	...
	if e.PrevExist != nil && !(*e.PrevExist) {
		if e.PrevValue != nil {
			return etcd.CreateError("prevValue", errors.New("conflicted preconditions: `prevExist=false` can't go with `prevValue`"))
		}
		if e.PrevIndex != nil {
			return etcd.CreateError("prevIndex", errors.New("conflicted preconditions: `prevExist=false` can't go with `prevIndex`"))
		}
	}

	// index 0 is meaningless in Raft
	if e.PrevIndex != nil && *e.PrevIndex <= 0 {
		return etcd.CreateError("prevIndex", errors.New("`prevIndex` must be larger than 0"))
	}

	if e.Refresh != nil && *e.Refresh {
		if e.Value != nil {
			return etcd.CreateError("value", errors.New("`refresh=true` must not go together with `value`"))
		}
		if e.Ttl == nil {
			return etcd.CreateError("ttl", errors.New("`refresh=true` must go together with `ttl`"))
		}
	} else {
		if e.Value == nil {
			return etcd.CreateError("value", errors.New("must provide `value`"))
		}
	}

	if e.Ttl != nil && *e.Ttl <= 0 {
		return etcd.CreateError("ttl", errors.New("`ttl` must be larger than 0"))
	}
...
func (e EtcdCommandRequest) ValidateDelete() error {
	if e.Action != strings.ToLower(http.MethodDelete) {
		return etcd.CreateError("action", errors.New("`action` must be `delete`"))
	}
