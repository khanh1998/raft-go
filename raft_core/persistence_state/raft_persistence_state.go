...
type RaftPersistenceStateImpl struct {
	...
	latestSnapshot gc.SnapshotMetadata // default, last log index and term are zeros
...
func (r *RaftPersistenceStateImpl) cleanupSnapshot(ctx context.Context, sm gc.SnapshotMetadata) (err error) {
	...
	for _, fileName := range fileNames {
		if (gc.IsSnapshotFile(fileName) || gc.IsTmpSnapshotFile(fileName)) &&
...
func (r *RaftPersistenceStateImpl) StreamSnapshot(ctx context.Context, sm gc.SnapshotMetadata, offset int64, maxLength int) (data []byte, eof bool, err error) {
	fileName := sm.FileName
...
func (r *RaftPersistenceStateImpl) CommitSnapshot(ctx context.Context, sm gc.SnapshotMetadata) (err error) {
	...
	if r.latestSnapshot == (gc.SnapshotMetadata{}) {
...
func (r *RaftPersistenceStateImpl) ReadLatestSnapshot(ctx context.Context) (snap gc.Snapshot, err error) {
	r.lock.RLock()
...
func (r *RaftPersistenceStateImpl) GetLog(index int) (gc.Log, error) {
	r.lock.RLock()
...
func (r *RaftPersistenceStateImpl) AppendLog(ctx context.Context, logItems []gc.Log) (index int, err error) {
...
func (r *RaftPersistenceStateImpl) DeleteLogFrom(ctx context.Context, index int) (deletedLogs []gc.Log, err error) {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *RaftPersistenceStateImpl) GetLatestSnapshotMetadata() (snap gc.SnapshotMetadata) {
	return r.latestSnapshot
}

func (r *RaftPersistenceStateImpl) trimPrefixLog(newSnapshot gc.SnapshotMetadata) {
	base := r.latestSnapshot.LastLogIndex
...
func (n *RaftPersistenceStateImpl) GetLastLog() (gc.Log, error) {
	n.lock.RLock()
	defer n.lock.RUnlock()
...
func (r *RaftPersistenceStateImpl) Deserialize(keyValuePairs []string, snapshot gc.SnapshotMetadata) (lastLogIndex int, err error) {
	r.lock.RLock()
	defer r.lock.RUnlock()
