...
type RPCProxyImpl struct {
	...
	accessible           bool               // this is outdated, to prevent early access when the node is still setting up
	...
	simulation *common.NetworkSimulation
}
...
func (r *RPCProxyImpl) stopConnections(ctx context.Context) {
	...
	r.log().InfoContext(ctx, "RPCProxy listener stop")
}
...
func (r *RPCProxyImpl) SetNetworkSimulation(network common.NetworkSimulation) {
	r.simulation = &network
}

func (r *RPCProxyImpl) UnsetNetworkSimulation() {
	r.simulation = nil
}

func (r *RPCProxyImpl) GetNetworkSimulation() *common.NetworkSimulation {
	return r.simulation
}
