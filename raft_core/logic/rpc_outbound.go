		...
		go func(peerID int) {
			...
			if err == nil {
				...
				if _, ok := n.nextOffset[peerID]; !ok {
					...
					n.nextOffset[peerID] = NextOffset{
						Offset:   0,
						FileName: gc.NewSnapshotFileName(sm.LastLogTerm, sm.LastLogIndex),
						Snapshot: sm,
					}
				}
			} else {
			...
			if _, ok := n.nextOffset[peerID]; ok {
				...
				if err != nil {
					...
					if output.Success {
						n.matchIndex[peerID] = gc.Min(n.nextIndex[peerID], n.GetLogLength())
						n.nextIndex[peerID] = gc.Min(n.nextIndex[peerID]+1, n.GetLogLength()+1) // data race
					} else {
						if output.Term <= currentTerm {
							n.nextIndex[peerID] = gc.Max(0, n.nextIndex[peerID]-1) // data race
						} else {
