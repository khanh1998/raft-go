...
type MembershipManager interface {
	...
	AddServer(input gc.AddServerInput) error
}
...
type RaftPersistenceState interface {
	AppendLog(ctx context.Context, logItems []gc.Log) (index int, err error)
	DeleteLogFrom(ctx context.Context, index int) (deletedLogs []gc.Log, err error)
	...
	StreamSnapshot(ctx context.Context, sm gc.SnapshotMetadata, offset int64, maxLength int) (data []byte, eof bool, err error)
	InstallSnapshot(ctx context.Context, fileName string, offset int64, data []byte) (err error)
	CommitSnapshot(ctx context.Context, sm gc.SnapshotMetadata) (err error)
	...
	GetLog(index int) (gc.Log, error)
	GetLastLog() (gc.Log, error)
	GetLatestSnapshotMetadata() (snap gc.SnapshotMetadata)
}
...
type MemberChangeSubscriber interface {
	...
	InformMemberChange(ctx context.Context, members map[int]gc.ClusterMember)
}
...
type RPCProxy interface {
	...
	SendPing(ctx context.Context, peerId int, timeout *time.Duration) (res gc.PingResponse, err error)
...
func NewRaftBrain(params NewRaftBrainParams) (*RaftBrainImpl, error) {
	...
	n := &RaftBrainImpl{
		id: params.ID,
		state: func() gc.RaftState {
			if params.CachingUp {
				return gc.StateCatchingUp
			}
			return gc.StateFollower
		}(),
