		...
		},
		{
			...
			n: RaftBrainImpl{
				persistState: persistence_state.NewRaftPersistenceState(persistence_state.NewRaftPersistenceStateParams{
					CurrentTerm: 3,
					VotedFor:    0,
					Logs:        []gc.Log{common.MockedLog{Term: 1}, common.MockedLog{Term: 2}, common.MockedLog{Term: 3}},
					Storage: storage.NewStorageForTest(
						storage.NewStorageParams{WalSize: 1024, DataFolder: "data/", Logger: logger},
						storage.NewFileWrapperMock(),
					),
...
func Test_nodeImpl_AppendEntries(t *testing.T) {
	...
	classicLogFactory := common.MockedLogFactory{
	...
	testCases := []TestCase{
		...
		},
		{
			...
			ps: persistence_state.NewRaftPersistenceState(persistence_state.NewRaftPersistenceStateParams{
				CurrentTerm: 2,
				Logs:        []gc.Log{common.MockedLog{Term: 1}, common.MockedLog{Term: 1}},
				Storage: storage.NewStorageForTest(
					storage.NewStorageParams{WalSize: 1024, DataFolder: "data/", Logger: logger},
					storage.NewFileWrapperMock(),
				),
		...
		},
		{
			...
			ps: persistence_state.NewRaftPersistenceState(persistence_state.NewRaftPersistenceStateParams{
				CurrentTerm: 2,
				Logs:        []gc.Log{common.MockedLog{Term: 1}},
				Storage: storage.NewStorageForTest(
					storage.NewStorageParams{WalSize: 1024, DataFolder: "data/", Logger: logger},
					storage.NewFileWrapperMock(),
				),
		...
		},
		{
			...
			ps: persistence_state.NewRaftPersistenceState(persistence_state.NewRaftPersistenceStateParams{
				...
				CurrentTerm: 3,
				Logs: []gc.Log{
					common.MockedLog{Term: 1, Command: "set x 5"},
					common.MockedLog{Term: 2, Command: "set x 5"},
					common.MockedLog{Term: 2, Command: "set x 5"},
				},
				Storage: storage.NewStorageForTest(
					storage.NewStorageParams{WalSize: 1024, DataFolder: "data/", Logger: logger},
		...
		},
		{
			...
			ps: persistence_state.NewRaftPersistenceState(persistence_state.NewRaftPersistenceStateParams{
				...
				Logs: []gc.Log{
					common.MockedLog{Term: 1, Command: "set x 5"},
					common.MockedLog{Term: 2, Command: "set y 5"},
				},
				Storage: storage.NewStorageForTest(
					storage.NewStorageParams{WalSize: 1024, DataFolder: "data/", Logger: logger},
					storage.NewFileWrapperMock(),
	...
	for index, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("[%d] %s", index, testCase.name), func(t *testing.T) {
			...
			testCase.n.stateMachine = common.MockedStateMachine{}
