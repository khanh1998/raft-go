	...
	tests := []struct {
	...
	}{
		{
			...
			want: fields{
				keyWatch: map[string]map[int]WatchRequest{
					"count": {
						1: {WaitIndex: 0, Channel: make(chan EtcdResultRes, 1)},
					},
				},
				prefixWatch: map[string]map[int]WatchRequest{},
			},
		},
		{
			...
			fields: fields{
				...
				prefixWatch: map[string]map[int]WatchRequest{},
				eventCache: []EtcdResultRes{
					{Action: "set", Node: KeyValue{Key: "count", CreatedIndex: 4, ModifiedIndex: 4}},
					{Action: "set", Node: KeyValue{Key: "count", CreatedIndex: 4, ModifiedIndex: 6}},
				},
				baseIndex: 0,
				capacity:  4,
			},
		...
		},
		{
			...
			want: fields{
				keyWatch: map[string]map[int]WatchRequest{
					"name": {
						1: {WaitIndex: 4, Channel: make(chan EtcdResultRes, 1)},
					},
				},
				prefixWatch: map[string]map[int]WatchRequest{},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			w := Watcher{
				...
				cache: WatcherCache{
					eventCache: make([]EtcdResultRes, 0, tt.fields.capacity),
...
func TestWatcher_Notify(t *testing.T) {
	...
	type args struct {
		newData EtcdResultRes
	}
	fakeChannel := make(chan EtcdResultRes, 10)
	tests := []struct {
		name        string
		fields      fields
		args        args
	...
	}{
		{
			...
			args: args{
				newData: EtcdResultRes{
					Action:   "set",
					Node:     KeyValue{Key: "count", CreatedIndex: 5, ModifiedIndex: 6},
					PrevNode: KeyValue{Key: "count", CreatedIndex: 5, ModifiedIndex: 5},
				},
			},
			want: fields{
				keyWatch: map[string]map[int]WatchRequest{
