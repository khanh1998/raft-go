...
func (w *Watcher) Register(key string, isPrefix bool, waitIndex int) (id int, channel EtcdResultPromise, err error) {
	w.lock.Lock()
	defer w.lock.Unlock()
	...
	if waitIndex > 0 {
		...
		// check data empty
		if found {
			channel = make(chan EtcdResultRes, 1)
			defer func() {
				channel <- resData
			}()
...
func (w *Watcher) Notify(newData EtcdResultRes) {
	w.lock.RLock()
