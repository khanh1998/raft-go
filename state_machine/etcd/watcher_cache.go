...
func NewWatcherCache(capacity int) WatcherCache {
	return WatcherCache{
		eventCache: make([]EtcdResultRes, 0, capacity),
		baseIndex:  0,
	}
}
...
func (w *WatcherCache) latest() (newData EtcdResultRes, ok bool) {
	w.lock.RLock()
	defer w.lock.RUnlock()
...
func (w *WatcherCache) oldest() (newData EtcdResultRes, ok bool) {
	w.lock.RLock()
	defer w.lock.RUnlock()
...
func (w *WatcherCache) find(key string, waitIndex int) (EtcdResultRes, bool) {
	w.lock.RLock()
	defer w.lock.RUnlock()

	if w.empty() || waitIndex == 0 {
		return EtcdResultRes{}, false
	}
...
func (w *WatcherCache) Get(key string, waitIndex int) (EtcdResultRes, bool, error) {
	w.lock.RLock()
	defer w.lock.RUnlock()

	if w.empty() || waitIndex == 0 {
		return EtcdResultRes{}, false, nil
	}
	...
	if waitIndex < oldest.Node.ModifiedIndex {
		return EtcdResultRes{}, false, EtcdResultErr{
			ErrorCode: 401,
...
func (w *WatcherCache) Put(result EtcdResultRes) {
	w.lock.Lock()
	defer w.lock.Unlock()
